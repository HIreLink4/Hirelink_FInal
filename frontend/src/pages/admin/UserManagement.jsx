...
const UserManagement = () => {
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`WARNING: Are you sure you want to PERMANENTLY delete "${name}"? This action CANNOT BE UNDONE and will remove ALL user data from the database.`)) return
    try {
              ...
              {users.map(u => (
                <tr key={u.userId} className={`border-b border-gray-50 hover:bg-gray-50 ${u.isDeleted ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-4 py-3 font-medium text-gray-800">
                    {u.name}
                    {u.isDeleted && u.accountStatus !== 'BANNED' && <span className="ml-2 px-2 py-0.5 rounded text-[10px] font-semibold bg-gray-200 text-gray-500 uppercase">Deleted</span>}
                    {u.isDeleted && u.accountStatus === 'BANNED' && <span className="ml-2 px-2 py-0.5 rounded text-[10px] font-semibold bg-red-100 text-red-600 uppercase">Banned</span>}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusBadgeClass(u.accountStatus, u.isDeleted)}`}>
                      {u.accountStatus === 'BANNED' ? 'BANNED' : (u.isDeleted ? 'DELETED' : u.accountStatus)}
                    </span>
                  </td>
                   ...
                   <td className="px-4 py-3">
                    <div className="flex gap-2">
                      {u.isDeleted ? (
                        <>
                          <button 
                            onClick={() => u.accountStatus === 'BANNED' ? handleUnban(u.userId) : handleRestore(u.userId)} 
                            className="px-3 py-1.5 bg-green-500 text-white text-xs rounded-lg hover:bg-green-600 transition-colors"
                          >
                            {u.accountStatus === 'BANNED' ? 'Unban' : 'Restore'}
                          </button>
                          <button 
